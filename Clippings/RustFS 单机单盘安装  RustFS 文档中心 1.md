---
title: "RustFS 单机单盘安装 | RustFS 文档中心"
source: "https://docs.rustfs.com.cn/installation/linux/single-node-single-disk.html"
author:
  - "[[RustFS]]"
...
本文档包含了 RustFS 单机多盘部署模式。

1. 请明确您的三种安装启用模式：
	- [单机单盘模式（SNSD）](https://docs.rustfs.com.cn/installation/linux/single-node-single-disk.html) (当前文档)
	- [单机多盘模式（SNMD）](https://docs.rustfs.com.cn/installation/linux/single-node-multiple-disk.html)
	- [多机多盘模式（MNMD）](https://docs.rustfs.com.cn/installation/linux/multiple-node-multiple-disk.html)
2. [安装前检查](https://docs.rustfs.com.cn/installation/checklists/) ，确保各项指标符合生产指导特征，若不需要生产标准可不阅读此指导；
...
![RustFS Single Node Single Disk Mode](https://docs.rustfs.com.cn/assets/single-node-single-disk.DbEq65Q6.jpg)
...
```bash
bashfirewall-cmd --zone=public --add-port=9000/tcp --permanent
...
1. 创建存储目录
```bash
bashsudo mkdir -p /data/rustfs0 /var/logs/rustfs /opt/tls
...
1. 创建 systemd 服务文件
```bash
bashsudo tee /etc/systemd/system/rustfs.service <<EOF
...
1. 启动服务并设置开机自启
```bash
bashsudo systemctl enable --now rustfs
```
1. 验证服务状态
```bash
bashsystemctl status rustfs
```
1. 检查服务端口
```bash
bashnetstat -ntpl
...
1. 查看日志文件
```bash
bashtail -f /var/logs/rustfs/rustfs*.log
```
1. 访问访问控制台

输入服务器的IP地址和端口，尝试访问访问控制台，查看到的界面如下：

![Console](https://docs.rustfs.com.cn/assets/console.ybdjaZ1d.jpg)